...
const search = document.querySelector('.out-search');

search.addEventListener('click', (event) => {
event.preventDefault();
console.log("suliman");
const subjectname=document.querySelector(".new-search-In").value;
console.log(subjectname);
fetch(`http://127.0.0.1:8000/school/commom/searchsubject/?subject_name=${subjectname}`, {
   headers: {
...
event.preventDefault();
fetch(`http://127.0.0.1:8000/school/check_enrollment/${localStorage.getItem('id')}/${courseItemDiv.getAttribute('course_id')}`, {
   headers: {
   'Authorization': `Bearer ${localStorage.getItem('token')}`,
   }
   })
